...
export function DetailModal({ subject, allSubjects, onClose, onEdit }: {
  ...
  const goToTab = (key: typeof activeTab) => {
    const from = tabKeys.indexOf(activeTab)
    const to = tabKeys.indexOf(key)
      ...
      >
        ...
        <div className="dm__header">
          ...
          <div className="dm__header-actions">
            <button
              className="dm__edit-btn"
              onClick={() => { handleClose(); setTimeout(onEdit, 280) }}
            >
              <Edit size={11} color="currentColor" /> Editar
            </button>
            <button
              className="dm__close-btn"
              onClick={handleClose}
            >
              <X size={14} />
            </button>
        ...
        {tabs.length > 1 && (
          <div className="dm__tabs">
            {tabs.map(t => (
              <button
                key={t.key}
                className={`dm__tab${activeTab === t.key ? ' dm__tab--active' : ''}`}
        ...
        <div className="dm__body">
          <div
            className={`dm__tab-content dm__tab-content--${tabDir > 0 ? 'right' : 'left'}`}
            key={activeTab}
          >
            {activeTab === 'info' && (<>
  ...
  return (
    <>
      ...
      >
        ...
        {compact && hovered && (
          <div className="subject-tooltip subject-tooltip--visible">
