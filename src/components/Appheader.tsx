        ...
        <div className="hdr-menu__section">
            ...
            <div className="hdr-cfg__field">
                ...
                <div className="hdr-cfg__year-row">
                    {[1, 2, 3, 4, 5, 6].map(y => <button key={y} type="button" className={`hdr-cfg__yr-btn${currentYear === y ? ' active' : ''}`} onClick={() => setCurrentYear(y)}>{y}°</button>)}
                </div>
                <div className="hdr-cfg__year-row" style={{ marginTop: 6 }}>
                    {([1, 2] as const).map(s => <button key={s} type="button" className={`hdr-cfg__yr-btn${currentSemester === s ? ' active' : ''}`} onClick={() => setCurrentSemester(s)}>{s}° cuatri</button>)}
                </div>
            </div>
            <div className="hdr-cfg__field">
            ...
            <div className="hdr-cfg__field">
                ...
                <div className="hdr-cfg__dur-row">
                    ...
                    <button type="button" className={`hdr-cfg__sem-btn${extraSemesters === 0 ? ' active' : ''}`} onClick={() => setExtraSemesters(0)}>sin extra</button>
                    <button type="button" className={`hdr-cfg__sem-btn${extraSemesters === 1 ? ' active' : ''}`} onClick={() => setExtraSemesters(1)}>+1 cuatri</button>
                </div>
            ...
            <div className="hdr-cfg__actions">
                <button className="hdr-cfg__btn" onClick={onClose} disabled={saving}>Cancelar</button>
...
function EditProfilePanel({ profile, onUpdateProfile, onUpdatePassword, onDeleteAccount, onClose, onSignOut }: {
    ...
    return (
        <div className="hdr-menu__section">
            ...
            {deleteConfirm && <button className="hdr-cfg__btn" style={{ marginTop: '6px' }} onClick={() => { setDeleteConfirm(false); setDeleteMsg(null) }}>Cancelar</button>}
        </div>
    )
}
