...
function gradeColor(g: number) {
    if (g >= 8) return '#4ade80'
    if (g >= 6) return '#86efac'
    ...
    return (
        <div className="an-donut-wrap">
            <svg width="120" height="120" viewBox="0 0 120 120">
                ...
                <text x={cx} y={cy - 4} textAnchor="middle" fill="#fff" fontSize="18" fontWeight="700" fontFamily="DM Sans">{total}</text>
                <text x={cx} y={cy + 14} textAnchor="middle" fill="#44445a" fontSize="9" fontFamily="DM Sans" letterSpacing="1">MATERIAS</text>
            </svg>
    ...
    return (
        <div className="an-ring-wrap">
            <svg width="88" height="88" viewBox="0 0 88 88">
                ...
                />
                <text x={cx} y={cy + 2} textAnchor="middle" fill="#fff" fontSize="14" fontWeight="700" fontFamily="DM Sans">
                    {Math.round(pct * 100)}%
                </text>
            </svg>
...
export default function AnalyticsModal({ subjects, careerName, currentYear, totalSubjects: cfgTotal, onClose }: Props) {
    ...
    const stats = useMemo(() => {
        ...
        const highGrades = grades.filter(g => g >= 8).length
            ...
            <div className="an-modal">
                ...
                <div className="an-body">
                    ...
                    <div className="an-kpi-row">
                        ...
                        <div className="an-kpi">
                            <span className="an-kpi-value" style={{ color: stats.gpa != null ? gradeColor(stats.gpa) : '#44445a' }}>
                                {stats.gpa ?? '—'}
                            </span>
                        ...
                        <div className="an-kpi-divider" />
                        <div className="an-kpi">
                            <span className="an-kpi-value" style={{ color: stats.highGrades > 0 ? '#a78bfa' : '#44445a' }}>{stats.highGrades}</span>
                            <span className="an-kpi-label">Nota ≥ 8</span>
                        </div>
                    </div>
                                ...
                                {stats.recentSems.map((k, i) => {
                                    ...
                                    return (
                                        <div key={k} className="an-vel-col">
                                            <span className="an-vel-count" style={{ color: val > 0 ? '#6366f1' : '#44445a' }}>{val}</span>
                                        ...
                                        <div key={y} className={`an-year-row${isActive ? ' an-year-row--active' : ''}`}>
                                            <div className="an-year-meta">
                                                ...
                                                <span className="an-year-stats">
                                                    ...
                                                    <span style={{ color: '#44445a' }}> / {d.total}</span>
                                                    {yearAvg != null && (
                                ...
                                <div className="an-section">
                                    ...
                                    <div className="an-rank-list">
                                        {stats.worst5.map((s, i) => (
                                            <div key={s.id} className="an-rank-row">
                                                <span className="an-rank-pos" style={{ color: '#44445a' }}>{i + 1}</span>
                                                <span className="an-rank-name">{s.name}</span>
                                                <span className="an-rank-grade" style={{ color: gradeColor(s.grade!) }}>{s.grade}</span>
                                            </div>
                                        ))}
